...
class Assignment(db.Model):
    ...
    def upsert(cls, assignment_new: 'Assignment'):
        if assignment_new.content is None:
             abort(400, description="Content cannot be None")
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        if(auth_principal.principal_id is None):
            ...
            assertions.assert_valid(assignment.state in [AssignmentStateEnum.SUBMITTED, AssignmentStateEnum.GRADED],  'Assignment must be submitted before it can be graded.')  
